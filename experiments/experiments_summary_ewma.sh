...
OUTPUT_FILE="results/results_ewma_$TIMESTAMP.csv" 
OUTPUT_SUMMARIZED_FILE="results/results_summarized_ewma_$TIMESTAMP.csv" 
TMP_SQL_FILE="queries_ewma.sql" 

cat <<EOT > $TMP_SQL_FILE
CREATE OR REPLACE TABLE detailed_experiments AS
SELECT
    *,
    split_part(split_part(filename, 'ewma\', 2), '__', 1) AS dataset,
    split_part(split_part(filename, 'ewma_', 2), '__swap', 1) AS ewma,
    split_part(split_part(filename, 'swap_', 2), '_ngrams', 1) AS swap,
    ROW_NUMBER() OVER () rn
FROM
    read_csv_auto('output/ewma/**', delim=',', header=FALSE, filename=TRUE);
