...
OUTPUT_FILE="results/results_noise_$TIMESTAMP.csv" 
OUTPUT_SUMMARIZED_FILE="results/results_summarized_noise_$TIMESTAMP.csv" 
TMP_SQL_FILE="queries_noise.sql" 

cat <<EOT > $TMP_SQL_FILE
CREATE OR REPLACE TABLE detailed_experiments AS
SELECT
    *,
    split_part(split_part(filename, 'noise\', 2), '__', 1) AS dataset,
    split_part(split_part(filename, 'noise_', 2), '__algo', 1) AS noise,
    split_part(split_part(filename, 'algo_', 2), '__swap', 1) AS algo,
    split_part(split_part(filename, 'swap_', 2), '_3_0.3', 1) AS swap,
    ROW_NUMBER() OVER () rn
FROM
    read_csv_auto('output/noise/**', delim=',', header=FALSE, filename=TRUE);
...
ORDER BY dataset, algo, noise, swap;
